...
public enum Rotation {
    ...
    public static List<Rotation> findRotation(Direction first, Direction second) {
        if (isRight90OrLeft270(first, second, EAST, SOUTH, WEST, NORTH)) {
            return List.of(R90, L270);
        } else if (isRight90OrLeft270(first, second, SOUTH, WEST, NORTH, EAST)) {
            return List.of(R180, L180);
        } else if (isRight90OrLeft270(first, second, WEST, NORTH, EAST, SOUTH)) {
            return List.of(R270, L90);
        }
        return List.of();
    }

    private static boolean isRight90OrLeft270(Direction first, Direction second, Direction... directions) {
